...
| **手機版 Milestone 2 第二步：收藏功能** | ✅ 完成程式碼＋**使用者已在模擬器實測確認收藏功能與帳號切換皆正常**（2026-08-21，見 commit a5b8ae5 訊息） | 2026-08-21（新一輪對話）使用者要求接續登入功能做收藏。**後端沒有新增任何東西**——`favorite.*`（`toggle`/`isFavorited`/`list`，皆 `protectedProcedure`）在網頁版個人頁面那輪早就做好，手機版的 `useAuth`／`Authorization: Bearer` context 已經能餵給既有的 `protectedProcedure`，純粹是手機版畫面/互動的工作，完全比照上一步「Google 登入」進度表條目末段列出的範圍。**Hooks**（`apps/mobile/src/hooks/`，比照網頁版 `useFavoriteStatus.ts`/`useToggleFavorite.ts`/`useFavorites.ts` 但把 `useSession()`（next-auth）換成手機版自己的 `useAuth()`，`enabled` 條件從 `status === "authenticated"` 改成 `status === "signedIn"`）：`useFavoriteStatus.ts`、`useToggleFavorite.ts`、`useFavorites.ts`（吃 `page` 參數）。**元件**：新增 `src/components/FavoriteButton.tsx`——不像網頁版拆三個 if-return 分支，改成單一 return 用 `disabled`/`filled` 兩個算出來的布林值決定樣式，`onPress` 依 `status` 分流（`signedOut` 觸發 `signInWithGoogle()`；`signedIn` 呼叫 `toggle` mutation，`onSuccess` invalidate `favorite.isFavorited`/`favorite.list`，寫法跟網頁版 `FavoriteButton.tsx` 的 `{ restaurantId, isFavorited: !isFavorited }` 目標狀態語意一致）；重用既有的 `HeartIcon`（`icons/Icons.tsx` 本來就有 `filled` prop，不用另外畫新圖示）。額外比照 `(tabs)/profile.tsx` 既有的 `handleUnauthorized` 慣例，在 query 回傳 `UNAUTHORIZED` 時清本機 session（手機版是自己簽的長效 JWT，跟網頁版的 next-auth session 不同，token 過期是真的會發生的情況，不是防禦性寫法）。**接上既有畫面**：首頁（`(tabs)/index.tsx`）的推薦卡片 header 列（`FriendlinessBadge` 旁邊）與完整列表卡片 footer 列（`FriendlinessBadge` 旁邊）都加了 `FavoriteButton`；詳情頁（`restaurant/[id].tsx`）標題區塊改成 `titleRow`（左邊店名/分類/地址/電話欄，右邊 `FavoriteButton`）。RN 的巢狀 `Pressable`（卡片本身可點擊進詳情頁，`FavoriteButton` 又是卡片內的 `Pressable`）不像網頁版 `<a>`包`<button>`會犯 HTML 規則，內層 `Pressable` 的觸控不會冒泡到外層，不需要像網頁版那樣把按鈕拆成 `Link` 的手足節點。**未登入時卡片上也會看到空心愛心按鈕**，點擊觸發 Google 登入，跟網頁版行為一致。**個人頁面加「我的收藏」**：把原本 `(tabs)/favorites.tsx` 的 `ComingSoonScreen` 佔位換成真正畫面——未登入顯示提示文字＋登入按鈕（比照 `(tabs)/profile.tsx` 的 `signedOut` 分支）；已登入時用 `useFavorites` 撈清單，卡片呈現重用跟首頁列表卡片同樣的排版（店名/分類/地址/`StatusTag`/`FriendlinessBadge`/`FavoriteButton`），清單超過一頁時用既有的 `Pagination` 元件（跟首頁列表分頁共用同一個元件，不重新做一套）。**驗證**：`yarn typecheck`（三個 workspace）／`yarn test`（`apps/web` 86＋`packages/shared` 101，沒有新增測試，因為後端沒有新程式碼，這輪純粹是手機版沒有測試骨架的畫面/互動）／`yarn lint`（根目錄跑 `apps/web`，另外 `cd apps/mobile && yarn lint` 跑 `expo lint`，兩邊都乾淨）／`npx expo export --platform ios`（Metro 打包成功，1535 個模組，沒有新增原生依賴，只是重用既有的 `react-native-svg` 圖示與既有元件）均綠燈。**Claude 完全沒辦法在本環境驗證模擬器/實機上的實際互動**，依專案 git 紀律停在工作目錄未 commit。**使用者本機實測後回報兩輪 401 問題**：第一輪是登入成功後所有 `favorite.*`／`user.getProfile` 查詢立刻 401 反覆重試；第一輪修法上線後，第二輪回報「重整模擬器→登入帳號 A→登出→登入帳號 B」時 B 登入成功後一樣立刻 401、卡住不動。追查發現是同一類問題的兩種manifestation（見「已知的坑」第 26 條完整說明），根本修法把 401 判斷邏輯從「每個元件各自用 `useEffect` 判斷 `error.data.code`」集中改成「`trpc.ts` 的 fetch 攔截器在網路層判斷：送出請求當下記住用的 token，收到 401 時比對現在的 token 是否還是同一個，只有真的還相關才通知 `useAuth.tsx` 唯一一份 `handleUnauthorized()`」，新增 `src/lib/unauthorizedHandler.ts` 做這層跨模組通知，移除了 `(tabs)/profile.tsx`／`FavoriteButton.tsx`／`(tabs)/favorites.tsx` 三處重複的 `useEffect` 判斷。已重新跑過 `yarn typecheck`／`yarn lint`（web＋mobile）／`npx expo export --platform ios` 確認乾淨。**使用者已在模擬器實測確認**：收藏功能與登出後立刻換帳號登入兩者皆正常，帳號切換不再卡在 401（server log 仍會看到 401，是舊帳號在飛行中的請求遲到才收到回應，屬於預期中的無害現象，詳見「已知的坑」第 26 條）。**尚待驗證的細項場景**：收藏在不同畫面間（首頁推薦卡片/完整列表卡片/詳情頁）互動是否一致、未登入點擊愛心按鈕導去登入的流程、個人頁面「我的收藏」分頁的分頁按鈕與移除收藏、深色/淺色主題下愛心圖示（實心/空心）與版面呈現。 |
| **手機版 Milestone 2 第三步：改名字＋大頭貼上傳** | ✅ 完成程式碼，⏸ **需要使用者重新原生建置＋模擬器/實機驗證**（2026-08-21） | 2026-08-21（新一輪對話）接續進度表第 18 項，使用者要求「完全比照網頁版」。**後端沒有新增任何東西**——`user.getProfile`/`user.updateName`/`user.updateAvatar` 三個 `protectedProcedure` 網頁版那輪早就做好（見上方「個人頁面：頭像上傳（裁切/縮放）＋改名字功能」與「修正大頭貼塞進 session cookie 導致 431」兩列），純粹是手機版畫面/互動的工作。**Hooks**（比照網頁版同名檔案，只把 `useSession()` 換成手機版 `useAuth()`／`status === "signedIn"`）：新增 `useUserProfile.ts`／`useUpdateUserName.ts`／`useUpdateUserAvatar.ts`。**EditableName.tsx**：邏輯完全比照網頁版（唯讀名稱＋編輯按鈕→輸入框＋儲存/取消，空白名稱前端擋下，儲存成功 `utils.user.getProfile.invalidate()`），UI 換成 RN 的 `TextInput`/`Pressable`。**大頭貼裁切的落地方式跟網頁版不同**：網頁版用 `react-easy-crop` 手刻拖曳/縮放的裁切彈窗，RN 沒有對應的成熟套件可以照搬；改用 `expo-image-picker` 的 `allowsEditing:true`＋`aspect:[1,1]`——觸發系統原生的相簿裁切 UI，一樣可以拖曳/縮放調整位置後再確認，只是彈窗本身是作業系統畫的不是自己刻的元件。選完的結果再用新增的 `expo-image-manipulator`（`ImageManipulator.manipulate(uri).resize({width:320,height:320}).renderAsync()` 這條新版 context-based API，不是已標記 deprecated 的 `manipulateAsync`）強制輸出成跟網頁版同規格的 320x320 JPEG base64 data URL（`src/lib/imageCrop.ts` 的 `processPickedImageToDataUrl`），跟網頁版 `cropImageToDataUrl` 控制資料量的用意一致。`AvatarUploader.tsx`：沒有大頭貼時顯示 `ProfileIcon`（重用既有的 `icons/Icons.tsx`，不用另外畫 UserIcon）圓形外框佔位，上傳中顯示 `ActivityIndicator`。**新增的其他 token**：`organicTheme.ts` 補上 `danger` 色票（light `#b3401f`／dark `#f0733d`，直接照抄網頁版 `globals.css` 的 `--danger` 數值，不是另外挑的顏色），給錯誤訊息文字用。**app.json** 新增 `expo-image-picker` config plugin（`photosPermission` 字串，會寫入 iOS `NSPhotoLibraryUsageDescription`），**個人頁面**（`(tabs)/profile.tsx` 的 `SignedInProfile`）換成 `<AvatarUploader />`＋`<EditableName />`，原本內嵌的 `trpc.user.getProfile.useQuery()`/`Image`/名稱 `Text` 都移除（各自元件內部自己查）。**過程中的坑**：`npx expo install expo-image-picker expo-image-manipulator` 再次把根目錄 `resolutions` 鎖的 React 版本改回 Expo 原本釘的 19.2.3（已知的坑第 22 條同一個問題），照舊解法回根目錄重跑一次 `yarn install` 拉回 19.2.8。**驗證**：`yarn typecheck`（三個 workspace）／`yarn test`（`apps/web` 86＋`packages/shared` 101，沒有新增測試，後端無新程式碼）／`yarn lint`（根目錄 web＋`cd apps/mobile && yarn lint`）／`npx expo export --platform ios`（Metro 打包成功，1551 個模組，含新增的 `expo-image-picker`/`expo-image-manipulator` 原生依賴）均綠燈。**Claude 完全沒辦法在本環境驗證原生相簿權限彈窗/系統裁切 UI 的實際手感/上傳後畫面**，依專案 git 紀律停在工作目錄未 commit。**新增了兩個原生依賴，使用者需要 `npx expo prebuild --clean` 後重新 `expo run:ios`/`expo run:android` 才會生效**（比照上一輪新增 `react-native-svg`/`expo-device` 時的流程），接著實機/模擬器測試：相簿權限請求彈窗、選圖後系統裁切 UI 拖曳/縮放手感、確認後大頭貼是否正確顯示處理後的結果、改名字流程（含空白名稱擋下、取消）、深色/淺色主題下的樣式呈現。 |
| **手機版：個人頁面加「編輯」切換按鈕＋單人座位回報表單（第一版）** | ✅ 完成程式碼，⏸ 使用者實測後回報兩個問題，見下一列的修正 | 2026-08-22（新一輪對話）兩個各自獨立的小需求。**(1) 個人頁面編輯按鈕**：使用者要求上傳大頭貼／編輯名稱兩顆按鈕預設收起，點一顆新的「編輯」按鈕才顯示。`AvatarUploader.tsx`／`EditableName.tsx` 各自新增 `showUploadButton`/`showEditButton` prop 控制「觸發編輯的按鈕」本身要不要顯示（頭貼圖片、名稱文字都不受影響，一直顯示）；`(tabs)/profile.tsx` 的 `SignedInProfile` 新增 `isEditMode` state 集中管理，往下傳給兩個子元件，並排一顆「編輯／完成」`Button`。**(2) 單人座位回報表單第一版**：詳情頁「單人友善」分頁原本只有一行靜態文字「登入後可以回報...這一輪還沒開放」，後端 `soloSeatReport.create`（`protectedProcedure`）網頁版那輪早就做好且在用，純粹是手機版沒接 UI——比照網頁版新增 `useSubmitSoloSeatReport.ts`與 `SoloSeatReportForm.tsx`（未登入顯示登入提示；已登入顯示「有/沒有單人座位」兩顆按鈕，**點擊即直接送出**＋選填備註），接掉 `restaurant/[id].tsx` 裡原本的靜態提示文字。**這版設計後來被使用者指出兩個問題（見下一列）**。 |
| **手機版＋網頁版：單人座位回報表單改成「唯讀摘要＋編輯」模式，並修正備註送出後看不到的問題** | ✅ 完成程式碼，⏸ **需要使用者在瀏覽器/模擬器實機驗證** | 2026-08-22（同一輪對話，使用者測第一版後截圖回報）兩個問題：**(1) 備註填了送出後畫面上完全看不到**——原設計送出後只是清空輸入框，沒有任何地方把使用者剛才寫的備註顯示回去，看起來像是「我填的東西消失了」。**(2)** 使用者明確要求「不要讓使用者點擊有單人座位或沒有的按鈕就可以改變單人座位信心％數，要有個明確的編輯（修改）跟新增（第一次）」——原設計兩顆按鈕點了就直接送出、直接改變信心分數，沒有二次確認、也沒有區分「這是第一次回報」還是「這是在改之前的回報」。**根因**：後端一直只有 `soloSeatReport.create`（upsert 寫入），沒有任何「查詢我自己對這間店的回報」的 procedure，前端無從得知使用者是否回報過、回報了什麼，UI 只能設計成「無狀態的兩顆按鈕」。**修法（改成比照 `EditableName.tsx` 的唯讀摘要＋編輯按鈕→表單＋送出/取消模式）**：**後端**新增 `soloSeatReport.getMine`（`protectedProcedure`，輸入 `restaurantId`）——`prismaClient.ts` 新增 `findSoloSeatReportByUserAndRestaurant`（用既有的 `@@unique([restaurantId, userId])` 索引 `findUnique`）、`soloSeatReportService.ts` 新增 `getMySoloSeatReport`（把 DB 理論上不會出現、但型別上存在的 `UNKNOWN` reportType 收斂成 `null`，前端不用處理不可能發生的第三種狀態）、`types/soloSeatReport.ts` 新增對應的 input schema／`MySoloSeatReport` 型別，`soloSeatReportService.test.ts` 補了 4 個測試（含比照已知的坑第 19 條的組合層轉呼叫斷言）。**網頁版與手機版的 `SoloSeatReportForm.tsx` 各自重寫**：未回報過時顯示「這間餐廳有單人座位嗎？」＋「新增回報」按鈕；已回報過時顯示唯讀摘要「你回報：有/沒有單人座位」＋備註（如果有，這裡就解決了問題 (1)，備註會持續顯示不會消失）＋「編輯」按鈕；點「新增回報」/「編輯」才進入表單模式，此時「有/沒有單人座位」變成單純的選項按鈕（選了不會送出，用 `selectedType` state 記錄目前選了哪個、用背景色標示已選取），編輯既有回報時會用查到的既有值預填選項與備註，一定要按下新增的「送出」按鈕才會真的呼叫 `create` mutation 改變信心分數，這就解決了問題 (2)；也新增「取消」按鈕退回唯讀摘要不送出。網頁版新增 `useMySoloSeatReport.ts`（比照 `useFavoriteStatus.ts` 用 `useSession()` 的 `status==="authenticated"` 控制 `enabled`），手機版同名檔案比照用 `useAuth()` 的 `status==="signedIn"`。送出成功時除了原本的 `restaurant.getById.invalidate()`，兩邊都新增 `soloSeatReport.getMine.invalidate()` 讓唯讀摘要立刻反映剛送出的內容。網頁版既有的 `SoloSeatReportForm.test.tsx` 整份改寫成涵蓋新流程的 7 個案例（未登入、新增回報選項预設不顯示直到點「新增回報」、選擇＋填備註＋送出才呼叫 mutate、已回報過顯示摘要含備註、點編輯會預填、取消不送出）。**過程中的坑**：手機版 `EditableName.tsx` 原本想用 `useEffect` 監看 `showEditButton` prop 反過來 `setIsEditing(false)`（上一版遺留），這次補測試時 `expo lint` 直接擋下 `react-hooks/set-state-in-effect`（React 官方不建議「用 effect 把 prop 變化同步回 state」）——改成移除該 effect，讓 `(tabs)/profile.tsx` 幫 `<EditableName>` 加上 `key={isEditMode ? "editing" : "view"}`，`isEditMode` 翻轉時整個元件重新掛載，state 自然回到初始值，不用手動同步。**驗證**：`yarn typecheck`（三個 workspace）／`yarn test`（`apps/web` 89＋`packages/shared` 105，新增 4+7=11 個測試）／`yarn lint`（根目錄 web＋`cd apps/mobile && yarn lint`）／`yarn build`（Next.js production build）／`npx expo export --platform ios`（Metro 打包成功，1554 個模組）均綠燈。**Claude 完全沒辦法在本環境驗證瀏覽器/RN 畫面互動**，依專案 git 紀律停在工作目錄未 commit，待使用者驗證：網頁版與手機版的新增回報／編輯回報／取消三個流程、備註是否正確顯示於唯讀摘要、送出後信心分數與回報則數是否即時更新、深色/淺色主題樣式。 |
| **手機版＋網頁版：單人座位回報新增「刪除」功能** | ✅ 完成程式碼，⏸ **需要使用者在瀏覽器/模擬器實機驗證** | 2026-08-22（同一輪對話接續）使用者要求「單人友善評論也要有刪除功能」。**後端**新增 `soloSeatReport.delete`（`protectedProcedure`，輸入 `restaurantId`）——`prismaClient.ts` 新增 `deleteSoloSeatReportTransaction`（跟 `submitSoloSeatReportTransaction` 同一套鎖 row／重算信心分數／寫回 Restaurant 三步驟，差別只在中間是 `deleteMany`（比照 `removeFavorite` 用 `deleteMany` 不用 `delete`，天生 idempotent，使用者本來沒回報過時重複呼叫也不會噴錯）而不是 `upsert`）、`soloSeatReportService.ts` 新增 `deleteSoloSeatReport` 組合層函式。`types/soloSeatReport.ts` 把原本只給 `getMine` 用的 `getMySoloSeatReportInputSchema` 重新命名成通用的 `soloSeatReportByRestaurantInputSchema`（`getMine`／`delete` 輸入形狀完全一樣，直接共用，不重複定義），`soloSeatReportService.test.ts` 補了 1 個組合層轉呼叫斷言測試。**前端**：網頁版新增 `useDeleteSoloSeatReport.ts`，唯讀摘要區塊（`myReport` 存在時）在「編輯」按鈕旁邊加一顆紅色「刪除」按鈕（`text-danger`／`hover:bg-danger/10`，跟 `AvatarUploader.tsx`/`EditableName.tsx` 既有的錯誤文字同一個 `--danger` 色票），點擊呼叫 `delete` mutation，成功後 invalidate `restaurant.getById`＋`soloSeatReport.getMine`（讓摘要立刻變回「尚未回報」狀態），`SoloSeatReportForm.test.tsx` 補了「尚未回報時不顯示刪除按鈕」＋「點刪除會呼叫正確參數並 invalidate 兩個快取」共 2 個案例。手機版同名 hook＋`SoloSeatReportForm.tsx` 比照邏輯，UI 不是重用共用的 `Button.tsx`（那個元件目前只有 primary/secondary/ghost 三種語意色，沒有 danger 色可用），改成跟 `EditableName.tsx` 既有的「編輯」/「取消」按鈕同一種手刻 `Pressable`+`Text` 寫法、文字與外框改用 `theme.danger`。**沒有加刪除前的二次確認彈窗**：比照這個專案「移除收藏」（`ProfileView.tsx` 的 `RemoveFavoriteButton`）既有的慣例，點擊即直接刪除，不彈確認視窗，維持跟既有刪除類互動一致的簡單度。**驗證**：`yarn typecheck`（三個 workspace）／`yarn test`（`apps/web` 90＋`packages/shared` 106）／`yarn lint`（根目錄 web＋`cd apps/mobile && yarn lint`）／`yarn build`（Next.js production build）／`npx expo export --platform ios`（Metro 打包成功）均綠燈。**Claude 完全沒辦法在本環境驗證瀏覽器/RN 畫面互動**，依專案 git 紀律停在工作目錄未 commit，待使用者驗證：刪除後畫面是否正確退回「尚未回報」狀態、信心分數/回報則數是否正確重算、深色/淺色主題下刪除按鈕（尤其手機版手刻的紅色外框按鈕）樣式是否正常。 |
| **手機版＋網頁版：修正回報成功後首頁/收藏清單信心分數沒跟著更新的快取不一致問題** | ✅ 完成程式碼，⏸ **需要使用者在瀏覽器/模擬器實機驗證** | 2026-08-22（同一輪對話接續）使用者截圖回報：首頁推薦卡片顯示「未知，建議致電確認 40」，同一間店的詳情頁卻顯示「已確認有單人座位／非常適合單人 100／單人座位信心：100%（2 則回報）」，並指出「信心狀態顯示不一致請使用適合此專案的狀態管理工具」。**根因**：這個專案的狀態管理本來就是 tRPC＋React Query（`trpc.useUtils()`），工具選對了，但上一輪新增回報/刪除功能時，成功後只 `invalidate` 了 `restaurant.getById`（詳情頁自己查的那個 procedure）跟 `soloSeatReport.getMine`——同一間餐廳的 `soloSeatStatus`/`soloSeatConfidence` 其實還分別快取在 `restaurant.pickOne`（首頁推薦卡片）、`restaurant.search`（完整列表）、`restaurant.mapMarkers`（地圖）、`favorite.list`（我的收藏）這幾個獨立的 query key 底下，只 invalidate `getById` 完全不會波及這些查詢，所以回報成功後只有詳情頁本身更新，切回首頁看到的還是回報前的舊資料。**修法**：`SoloSeatReportForm.tsx`（網頁版與手機版）新增 `invalidateAfterReportChange` 共用函式，改用 tRPC React Query 的 **router 層級 invalidate**——`utils.restaurant.invalidate()`（不帶特定 procedure，會連帶 invalidate `restaurant.*` 底下所有 query，不用每個查詢各自手動列一次，也不怕之後 `restaurant` 底下新增查詢又忘記補）＋`utils.favorite.list.invalidate()`，取代原本只 invalidate `restaurant.getById` 的寫法；新增/編輯/刪除三個成功路徑都改呼叫這個共用函式。網頁版 `SoloSeatReportForm.test.tsx` 兩個既有的 invalidate 斷言測試同步更新（改成斷言 `utils.restaurant.invalidate()`／`utils.favorite.list.invalidate()` 有被呼叫，不再斷言帶 `{id}` 參數的 `getById.invalidate`）。**驗證**：`yarn typecheck`（三個 workspace）／`yarn test`（`apps/web` 90＋`packages/shared` 106，無新增測試，純粹是既有測試斷言對象改變）／`yarn lint`（根目錄 web＋`cd apps/mobile && yarn lint`）／`yarn build`（Next.js production build）／`npx expo export --platform ios`（Metro 打包成功）均綠燈。**Claude 完全沒辦法在本環境驗證瀏覽器/RN 畫面互動**，依專案 git 紀律停在工作目錄未 commit，待使用者驗證：在詳情頁新增/編輯/刪除回報後，切回首頁推薦卡片、完整列表、「我的收藏」（如果這間店有被收藏）是否都同步顯示最新的信心分數與友善度徽章，不再看到跟詳情頁不一致的舊資料。 |
| **網頁版＋手機版：移除卡片/詳情頁「文字狀態」與「友善度徽章」重複顯示同一件事** | ✅ 完成程式碼，⏸ **需要使用者在瀏覽器/模擬器實機驗證** | 2026-08-22（同一輪對話接續，快取修好後使用者再截圖回報）兩個回報：(1) 重整後不同頁面信心度顯示仍不一致；(2) 新增回報後「未知，建議致電確認 40」這個狀態沒變；並指出文字狀態（如「尚未確認，建議致電詢問」）跟友善度徽章（如「未知，建議致電確認 40」）兩種資訊重複顯示，要求只留一種。**先排查 (1)(2) 是否還是 bug**：這兩個回報用的是同一間店只有 1 則回報的情境——`computeSoloSeatStatus`（`packages/shared/src/pure/soloSeatStatus.ts`）刻意設計「只有 1 筆回報時，即使同向也不下定論」（避免單一使用者洗成定論，`soloSeatReportService.test.ts` 本來就有測試涵蓋這個邊界），所以 `soloSeatStatus` 停在 `UNKNOWN`、對應的友善度分數固定 40，`soloSeatConfidence` 卻已經是 100%（1 則回報全部同向）——這是刻意的業務邏輯，不是快取或狀態管理的 bug，两個畫面數字沒有真的兜不起來，只是「狀態」跟「信心」兩個維度分開看容易誤讀成矛盾；上一輪的 invalidate 修法本身有效，這兩則回報不是快取問題復發。**真正要處理的是使用者明確指出的重複顯示**：讓使用者用 `AskUserQuestion` 選擇處理方式，選定「依情境分開」——清單/卡片類畫面（首頁推薦卡、完整列表、我的收藏）只留友善度徽章（`FriendlinessBadge`，適合快速掃視比較）；詳情頁只留文字狀態＋信心百分比/回報則數（`soloSeatStatusLabel`＋`單人座位信心：X%（N 則回報）`，資訊更完整精確）。**改動範圍（網頁版＋手機版對稱）**：`RestaurantDetailView.tsx`／`restaurant/[id].tsx` detail 頁的「單人友善」卡片移除 `FriendlinessBadge`；`page.tsx`（首頁 hero 卡＋完整列表卡）／`ProfileView.tsx`（收藏清單）／`(tabs)/index.tsx`（首頁 hero 卡＋完整列表卡）／`(tabs)/favorites.tsx`（收藏清單）都移除文字狀態，只留友善度徽章——列表卡片裡原本文字狀態後面接的 `soloSeatType`（座位類型描述，例如「吧台單人座」）不是重複資訊，改成有值才顯示、不再掛在狀態文字後面。手機版 `StatusTag.tsx` 移除後已無任何地方使用，直接刪除整個檔案（不留死代碼）；同步調整兩個 workspace 的 footer row 版面（原本左右兩側分別放狀態/徽章，現在只剩徽章＋收藏愛心，改成靠右對齊）。**網頁版測試同步更新**：`RestaurantDetailView.test.tsx` 改成斷言友善度徽章不顯示（新增 1 個測試）、`HomePage.test.tsx` 改成斷言文字狀態不顯示，兩處都留下註解說明這是刻意的重複顯示修正，不是誤刪。**驗證**：`yarn typecheck`（三個 workspace）／`yarn test`（`apps/web` 91＋`packages/shared` 106）／`yarn lint`（根目錄 web＋`cd apps/mobile && yarn lint`）／`yarn build`／`npx expo export --platform ios` 均綠燈。**Claude 完全沒辦法在本環境驗證瀏覽器/RN 畫面互動**，依專案 git 紀律停在工作目錄未 commit，待使用者驗證：各畫面版面在移除其中一種資訊後排版是否正常（尤其手機版 footer row 改成靠右對齊）、深色/淺色主題下友善度徽章／文字狀態的可讀性、只有 1 則回報時「未知 40」+「信心 100%」同時出現是否已經跟使用者說明清楚是預期行為而非 bug。 |
| **網頁版＋手機版：友善度徽章移除裸分數數字，只留文字標籤** | ✅ 完成程式碼，⏸ **需要使用者在瀏覽器/模擬器實機驗證** | 2026-08-22（同一輪對話接續，上一輪修完重複顯示後使用者接著問「40 是什麼意思」）Claude 解釋完 0-100 分友善度分數的算法（`computeSoloFriendlinessScore`：UNKNOWN 底分 40、CONFIRMED_YES 底分 70 起、CONFIRMED_NO 底分依信心分數往下扣，門檻對應到「非常適合單人/適合單人/未知建議致電確認/不建議單人前往」四個標籤）後，使用者認為 App 裡完全沒有任何地方說明這個數字的意思，使用者會看不懂，要求乾脆移除數字顯示。**改動範圍**：`FriendlinessBadge.tsx`（網頁版＋手機版）都拿掉顯示分數的那個 `<span>`/`<Text>`，只留文字標籤（如「非常適合單人」）；`score` prop 保留不拿掉，因為徽章底色深淺（`toneClassName`／`bg`/`fg`）還是依分數區間決定，只是不再把數字印出來，兩個檔案都補了註解說明原因（避免下次不知情又加回去）。手機版順便移除變成沒用到的 `score` 樣式（`fontSize:10, opacity:0.7`），`gap:4` 留著沒影響（單一子元素時沒有視覺作用，不影響正確性，沒必要為了這個額外改版面）。**驗證**：`yarn typecheck`（三個 workspace）／`yarn test`（`apps/web` 91＋`packages/shared` 106，無測試斷言到裸分數文字，不用改測試）／`yarn lint`（根目錄 web＋`cd apps/mobile && yarn lint`）／`yarn build`／`npx expo export --platform ios` 均綠燈。**Claude 完全沒辦法在本環境驗證瀏覽器/RN 畫面互動**，依專案 git 紀律停在工作目錄未 commit，待使用者驗證：各畫面徽章移除數字後版面/間距是否正常、深色淺色主題下徽章顏色是否還是依分數區間正確變化（即使數字本身看不到）。 |
| **UNKNOWN 狀態的友善度標籤加上「單人座位」，避免看起來不知道在講什麼** | ✅ 完成程式碼，⏸ **需要使用者在瀏覽器/模擬器實機驗證** | 2026-08-22（同一輪對話接續）使用者指出「未知，建議致電確認」這句話沒講清楚是在說什麼「未知」，要求改成能清楚知道是在說單人座位的狀態。其餘三個標籤（非常適合單人／適合單人／不建議單人前往）本來就有提到「單人」，只有 UNKNOWN 這一句沒有，所以只改這一句。`packages/shared/src/pure/restaurantFriendliness.ts` 的 `resolveSoloFriendlinessLabel` 把 `"未知，建議致電確認"` 改成 `"單人座位未知，建議致電確認"`，`restaurantSearchService.test.ts` 裡對應的 4 處斷言字串同步更新。**驗證**：`yarn typecheck`（三個 workspace）／`yarn test`（`apps/web` 91＋`packages/shared` 106）／`yarn lint` 均綠燈。**Claude 完全沒辦法在本環境驗證瀏覽器/RN 畫面互動**，依專案 git 紀律停在工作目錄未 commit，待使用者確認新標籤在徽章裡（尤其手機版寬度較窄的卡片）會不會太長、換行或截斷。 |
| **新增隱私權政策頁面（`/privacy`），並把網頁版個人頁面的編輯按鈕行為同步到手機版模式** | ✅ 完成程式碼，⏸ **需要使用者在瀏覽器驗證** | 2026-08-22。(1) 上架 App Store/Play Store 需要一個公開的隱私權政策網址，新增 `apps/web/src/app/privacy/page.tsx`（server component 靜態頁），內容依實際盤點程式碼會蒐集的資料寫成（Google 登入資料／大頭貼與暱稱編輯／地圖定位僅裝置端運算不落地／收藏／單人座位回報僅本人可見只有聚合分數公開／Google Places 第三方資料／無分析追蹤／JWT session cookie），聯絡信箱用使用者確認過的 `112182ssss@gmail.com`；因為目前沒有帳號自助刪除功能，政策裡寫「寄信申請刪除」，並提醒使用者 Apple Guideline 5.1.1(v) 通常要求 App 內建自助刪除，上架前可能要另外補。(2) 使用者接著要求網頁版個人頁面的「編輯」按鈕要跟手機版同步——手機版 `1b02cae` 那輪已經有「大頭貼上傳／改名字按鈕預設收起，點集中管理的『編輯』/『完成』按鈕才顯示」這個 UX（`profile.tsx` 的 `isEditMode` 往下傳給 `AvatarUploader`/`EditableName` 的 `showUploadButton`/`showEditButton` prop），但網頁版原本兩個按鈕永遠都顯示，沒有這層編輯模式。比照手機版把這個模式搬到網頁版：`AvatarUploader.tsx`/`EditableName.tsx` 都新增對應的 `showXxxButton` prop（`false` 時不 render 按鈕，頭貼圖片/名稱文字本身不受影響），`ProfileView.tsx` 新增 `isEditMode` state 集中管理，往下傳給兩個子元件，並新增「編輯」/「完成」切換按鈕（放在登出／主題切換那一列），`EditableName` 用 `key={isEditMode ? "editing" : "view"}` 在切回收起狀態時強制重新掛載重置內部編輯中的 state（比照手機版同一招，不用 `useEffect` 監看 prop 反過來 setState）。(3) 同一輪使用者追加要求：寬螢幕時大頭貼跟名稱要水平排列——原本大頭貼／名稱／信箱三者永遠 `flex-col` 直向堆疊，改成外層在 `sm:` 以上斷點變成 `flex-row`，大頭貼在左、名稱＋信箱一欄在右（`items-start`），窄螢幕維持原本置中直向堆疊。**測試**：`AvatarUploader.test.tsx`/`EditableName.test.tsx` 既有測試補上 `showUploadButton`/`showEditButton` 必填 prop（皆設 `true` 維持原本斷言的互動情境），各新增 1 個「prop 為 false 時按鈕不顯示」的測試；`ProfilePage.test.tsx` 的 `AvatarUploader`/`EditableName` stub 改成會把 prop 印成 `data-*` attribute 的版本（不再是純 `() => null`），新增 1 個「點擊編輯/完成會正確切換兩個子元件的 prop 與按鈕文字」的組合層轉傳測試（比照已知的坑「組合層轉傳測試」的方法論）。**驗證**：`yarn typecheck`（三個 workspace）／`yarn test`（`apps/web` 94＋`packages/shared` 106）／`yarn lint`／`yarn build`（`next build` 確認 `/privacy` 正確生成為靜態路由）均綠燈。**Claude 沒有在瀏覽器實際看過** `/privacy` 頁面的排版與深色/淺色主題可讀性、個人頁面編輯模式切換的互動、寬螢幕大頭貼與名稱水平排列的實際效果，依專案 git 紀律停在工作目錄未 commit，待使用者驗證後再協助 commit。 |
...
27. **登出時清 `favorite.list`／`favorite.isFavorited`／`user.getProfile` 快取，若當下剛好有
    掛載中的畫面在觀察這些 query，可能會多打一次沒帶 token 的請求、server log 出現一筆 401**
    （2026-08-22 加入，源自帳號切換快取外洩修法的副作用）。**背景**：這幾個 query 的 key
    沒有帶 `userId`，登出時如果只 `invalidate()`，帳號 A 登出、帳號 B 緊接登入時畫面可能會
    先閃一下 A 的舊快取。修法是在 `useAuth.tsx` 的 `clearLocalSession()` 裡改用
    `queryClient.removeQueries()` 把這幾個 query 的快取整筆刪除，且要在 `setStatus`
    ("signedOut") 之前完成。**已知的副作用**：如果清除快取當下，剛好有畫面（例如收藏分頁、
    個人頁面）正在掛載且這些 query 是 enabled 狀態，React Query 的 observer 偵測到資料被
    清空後會自動重新發出一次請求——這次請求的 token 已經被先清成 null（清除順序刻意如此，
    見 `clearLocalSession()` 內註解），所以會打出一個沒有 Authorization header 的請求，
    伺服器如實回 401，這筆 401 會出現在 server log。**這是預期中、無害的行為**：`trpc.ts`
    的 fetch 攔截器只在 `tokenForThisRequest !== null` 時才會呼叫
    `notifyUnauthorized()`（見已知的坑第 26 條），token 是 null 的請求不會觸發，不會有
    重複清除或迴圈的風險。跟第 26 條是同一類「server log 有 401 但 client 行為正確」的
    現象，只是觸發時機不同（第 26 條是舊帳號飛行中的延遲回應，這條是登出當下清快取觸發的
    主動重抓）——**之後如果又在 log 裡看到登出瞬間的 401，先確認是不是這個已知情境，不用
    當成新 bug 排查**。
...
17. ~~手機版 Milestone 2 第二步：收藏功能~~ 2026-08-21 程式碼完成，**使用者已在模擬器實測
    確認收藏功能與帳號切換皆正常**，仍有幾個細項場景待驗證，見上方進度對照表同名列
18. ~~手機版 Milestone 2 第三步：改名字＋大頭貼上傳~~ 2026-08-21 程式碼完成，
    ⏸ **需要使用者重新原生建置（新增了 expo-image-picker/expo-image-manipulator
    兩個原生依賴）並在模擬器/實機驗證**，見上方進度對照表同名列
19. ~~手機版 Milestone 2：單人座位回報表單~~ 2026-08-22 程式碼完成，⏸ **需要使用者在
    模擬器/實機驗證**，見上方進度對照表「手機版：個人頁面加「編輯」切換按鈕＋單人座位
    回報表單」列。手機版 Milestone 2 剩餘項目：地圖檢視（尚未規劃）
